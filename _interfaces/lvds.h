...
//Not sure about these transitions, something strange was happening. Maybe
//it was due to something else, I dunno.
// a/o v92: FULL_INIT_TESTS doesn't appear to be defined anywhere
//          But BOE is working...
//          (Was it a LTN thing?)
#warning "These transitions may have a flaw... a/o v91->v66.51 FULL_INIT_TESTS was necessary"
...
//This had to be added in v66.51, was not here, either (v92)
// Pasted commented-out as a reminder that, yes, in fact, FULL_INIT_TESTS
// is not used here and it seems to be working fine with BOE...
#define DEblue_fromNada()		DEblue_init()
//#error "FULL_INIT_TESTS is enabled... see notes re: its necessity in v66.51(v91?) and the note above. It should be fine to remove this error"
#else   //NOT FULL_INIT_TESTS
...
//v66.51: attempted to change these to OCR1A=6, didn't fix anything, so
//left-alone here (v92)
#define DEblue_fromNada()   { OCR1A=0xff; }
#define DEblue_fromDEonly() { OCR1A=0xff; }

//This just attempts to match the number of instructions...
#define DEblue_fromDEonly_Placeholder() { asm("nop;"); asm("nop;"); }
...
// (Forgot to set DT1, originally... see notes re FULL_INIT_TESTS found
// necessary in v66.51)
#define DEonly_fromDEblue() { DT1=(2<<4); OCR1A=4; }
//This just attempts to match the number of instructions...
#define DEonly_fromDEblue_Placeholder() { asm("nop;"); asm("nop;"); \
														asm("nop;"); asm("nop;"); }
...
//a/o v91 (66.51)
//The fact this doesn't work when initted as enableGreen
// (with e.g. BLUE_FRAME) leads me to
//believe something's still hanging around that affects the clock...
// (DT1?)
// (a/o v92: This note added to v66.51 when experimentation led to
// FULL_INIT_TESTS... The current state is it seems to work when
// enabled/disabled during e.g. BLUE_FRAME... See there (lcdStuff
// v0.80ncfDD6651)
...
//a/o v75:
// These are necessary to get v73-74 working again with the 861:
// (See above)
// These are NOT timing-precise, and shouldn't be used except in special
// cases... ish.
//   Red     OCR1D        Using:
//   0       0, 1, 2      2
//   1       3            3
//   2       4 (5?)       4
//   3       6+           7
#define setRed4(val) { OCR1D = ( ((val)<3) ? ((val)+2) : 7 ) ; }
 ...
 * /Users/meh/_avrProjects/LCDdirectLVDS/93-checkingProcessAgain/_interfaces/lvds.h
 *
