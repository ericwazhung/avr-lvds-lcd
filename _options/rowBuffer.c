...
// ALSO: MOST NOTES are re: Tiny861
...
uint8_t fb_to_rb(uint8_t fbColor)
{
	...
	//setRed4(val):   OCR0SA=(val+2)                            2,3,4,5
	//setGreen4(val): OCR0SB=(val+3)                            3,4,5,6
	//setBlue4(val):  OCR2RAL = (val == 3) ? 8 : (val + 4)      4,5,6,8

	//R: 2,3,4,5  10, 11, 100, 101 
	//G: 3,4,5,6  11, 100, 101, 110
	//            00  01   10   11        + 3
	//B: 4,5,6,8  100, 101, 110, 1000
	//            00   01   10   100      + 4
   uint8_t rb = 0;
   ...
   //        \_ _/ \_ _/ \ /
   //          V     V    V
   //          |     |    Green Setting   (writeColor needs to add 3)
   //          |     Red Setting          (direct)
   //          Blue Setting - 4           (writeColor needs to add 4)
...
#warning "This isn't at all optimized..."
   ...
   // Red setting
   uint8_t redValue = (fbColor & 0x03) + 2;
   rb |= (redValue << 2);
